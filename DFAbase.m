...
for i=1:size(t_fen,1) % Pour chaque taille de fenêtre
    for j=1:n_fen(i) % Pour chaque fenêtre de taille t_fen
        ...
        absc=1+t_fen(i)*(j-1):t_fen(i)*j; % vecteur des points en abscisses
        ...
        regLin(absc) = a*absc+b; % droite y = a*x+b; où x sont les points de absc 
    end
    ...
    F(i,:) = (bruitInt1(1:size(regLin,2))-regLin).^2;
    F2(i)=sqrt(sum(F(i,:))/x);
end
% Alpha 
%Passage au log
